...
/// # Parameters
///
/// This driver takes no parameters.
///
/// # Returns
///
...
mod tests {
    ...
    fn test_driver_metadata() {
        let schedule_driver = ScheduleTaskDriver;
        let unschedule_driver = UnscheduleTaskDriver;
        let list_driver = ListScheduledTasksDriver;
        assert_eq!(schedule_driver.name(), "schedule_task");
        assert_eq!(unschedule_driver.name(), "unschedule_task");
        assert_eq!(list_driver.name(), "list_scheduled_tasks");
        assert_eq!(schedule_driver.category(), DriverCategory::ScheduledTasks);
        assert_eq!(unschedule_driver.category(), DriverCategory::ScheduledTasks);
        assert_eq!(list_driver.category(), DriverCategory::ScheduledTasks);
        assert!(!schedule_driver.description().is_empty());
        assert!(!unschedule_driver.description().is_empty());
        assert!(!list_driver.description().is_empty());
        assert!(!schedule_driver.usage_hint().is_empty());
    ...
    fn test_driver_parameters() {
        ...
        let unschedule_driver = UnscheduleTaskDriver;
        let list_driver = ListScheduledTasksDriver;
        let schedule_params = schedule_driver.parameters();

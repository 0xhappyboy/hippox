...
pub fn launch_app(app_path: &str) -> DriverResult<u32> {
    ...
    let child = match crate::common::hidden_cmd(app_path).stdout(Stdio::null()).stderr(Stdio::null()).spawn() {
        Ok(c) => c,
        Err(e) => {
...
pub fn launch_app_with_args(app_path: &str, args: &[String]) -> DriverResult<u32> {
    ...
    let child = match crate::common::hidden_cmd(app_path).args(args).stdout(Stdio::null()).stderr(Stdio::null()).spawn() {
        Ok(c) => c,
        Err(e) => {
...
pub fn launch_as_admin(app_path: &str, args: &[String]) -> DriverResult<u32> {
    ...
    let mut cmd = crate::common::hidden_cmd("sudo");
    cmd.arg(app_path);
    cmd.args(args);
    let child = match cmd.spawn() {
        Ok(c) => c,
...
pub fn get_app_path(app_name: &str) -> DriverResult<String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Use where command
        if let Ok(output) = crate::common::hidden_cmd("where").arg(app_name).output() {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        if let Ok(output) = crate::common::hidden_cmd("which").arg(app_name).output() {
            if output.status.success() {

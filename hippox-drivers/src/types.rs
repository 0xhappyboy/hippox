use crate::{DriverCategory, DriverResult, DriverSignal};
...
impl DriverContext {
    ...
    pub fn with_task_id(task_id: impl Into<String>) -> Self {
        Self { task_id: Some(task_id.into()), ..Default::default() }
    }
    /// Get task_id
...
pub trait DriverCallback: Send + Sync + Debug {
    ...
    fn on_output(&self, task_id: Option<String>, driver_index: Option<usize>, driver_name: Option<String>, output: Option<String>) {}
    /// Progress update
...
pub trait Driver: Send + Sync + Debug {
    ...
    fn example_call(&self) -> DriverResult<serde_json::Value> {
        Ok(serde_json::json!({}))
    }
    /// Example output
    ///
    ...
    /// # Returns
    ///
    /// * `DriverResult<String>` - The Driver's output as a string, or an error
    ///
    /// # Errors
    ///
    ...
    /// # Returns
    ///
    /// * `DriverResult<()>` - Ok if validation passes, Err otherwise
    fn validate(&self, parameters: &HashMap<String, Value>) -> DriverResult<()> {
        ...
        for def in param_defs {
            ...
            if def.required && !has_value {
                return Err(crate::DriverError::MissingParameter { field: param_name.clone() });
            }
            if let Some(value) = parameters.get(param_name) {
                ...
                if !type_matches {
                    return Err(crate::DriverError::InvalidType {
                        field: param_name.clone(),
                        expected: def.param_type.clone(),
                        actual: format!("{:?}", value),
                    });
                }
                if let Some(enum_vals) = &def.enum_values {
                    if let Some(str_val) = value.as_str() {
                        if !enum_vals.contains(&str_val.to_string()) {
                            return Err(crate::DriverError::InvalidEnumValue {
                                field: param_name.clone(),
    ...
    fn get_metadata(&self) -> DriverMetadata {
        DriverMetadata {
            ...
            example_call: self.example_call().unwrap_or(serde_json::json!({})),
...
mod tests {
    ...
    impl Driver for TestDriver {
        ...
        async fn execute(
            ...
            let result = params.get("input").and_then(|v| v.as_str()).unwrap_or("no input");
        ...
        fn parameters(&self) -> Vec<DriverParameter> {
            return vec![
                DriverParameter {
                    ...
                    enum_values: Some(vec!["red".to_string(), "green".to_string(), "blue".to_string()]),
                },
                DriverParameter {
    ...
    async fn test_driver_metadata_creation() {
        let driver = TestDriver { name: "test_driver".to_string(), description: "A test driver".to_string() };
    ...
    async fn test_driver_execution_with_parameters() {
        let driver = TestDriver { name: "echo_Driver".to_string(), description: "Echoes input".to_string() };
    ...
    async fn test_driver_validation() {
        ...
        assert!(result.unwrap_err().to_string().contains("Required parameter 'color'"));
        let mut invalid_enum = HashMap::new();
        invalid_enum.insert("color".to_string(), json!("yellow"));
        let result = driver.validate(&invalid_enum);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("not in allowed values"));
    ...
    fn test_driver_call_deserialization() {
        ...
        assert_eq!(call.parameters.get("path").unwrap().as_str(), Some("./config.json"));
        assert_eq!(call.parameters.get("encoding").unwrap().as_str(), Some("utf-8"));
    }
    #[test]
    ...
    fn test_driver_parameter_with_enum_values() {
        let param = DriverParameter {
            ...
            enum_values: Some(vec!["fast".to_string(), "slow".to_string(), "balanced".to_string()]),
        };
        assert_eq!(param.enum_values.as_ref().unwrap().len(), 3);
        assert!(param.enum_values.as_ref().unwrap().contains(&"fast".to_string()));
    }
    #[test]
    fn test_context_new() {
    ...
    fn test_context_getters_setters() {
        ...
        ctx.set_task_id("task-456").set_driver_index(3).set_driver_name("download_file");
    ...
    fn test_context_extra_operations() {
        let mut ctx = DriverContext::new();
        ctx.insert_extra("url", json!("https://example.com")).insert_extra("timeout", json!(30));

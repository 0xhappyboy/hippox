...
pub(crate) fn get_identity_intro() -> String {
    ...
    if !has_name && !has_role && !has_personality && !has_species && !has_age && !has_sex && !has_tone && !has_catchphrase {
    ...
    if let Some(name) = &identity.name {
        intro.push_str(&format!("- You may call yourself \"{}\" in conversation.\n", name));
    }
    if let Some(age) = &identity.age {
        if let Some(species) = &identity.species {
            intro.push_str(&format!("- You can describe yourself as a {} year old {}.\n", age, species));
        } else {
    ...
    if let Some(sex) = &identity.sex {
        ...
        intro.push_str(&format!("- Your conversational gender reference is {}.\n", sex_word));
    }
    if let Some(role) = &identity.role {
        intro.push_str(&format!(
            "- In conversation, you can say you \"work as a {}\" (this is just roleplay, you are still a skill orchestration engine).\n",
            role
        ));
    }
    if let Some(personality) = &identity.personality {
        intro.push_str(&format!("- Your conversational tone should be {}.\n", personality));
    }
    if let Some(tone) = &identity.tone_style {
        intro.push_str(&format!("- Speak in a {} manner.\n", tone));
    }
    if let Some(catchphrase) = &identity.catchphrase {
        intro.push_str(&format!("- You may occasionally say \"{}\".\n", catchphrase));
    }

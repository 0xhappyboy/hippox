        ...
        Box::pin(async move {
            let intent_result = match pipeline.intent_analysis(&scheduler, &input, &task_id).await {
                ...
                Err(e) => {
                    ...
                    crate::pipeline::IntentAnalysisResult { categories: vec![], clean_intent: input.clone() }
                }
            };
            ...
            let (display_output, raw_json) = match &result {
                ...
                WorkflowExecutionResult::CompletedWithRaw { display, raw_json } => (display.clone(), raw_json.clone()),
                WorkflowExecutionResult::Paused { partial_output, .. } => (partial_output.clone(), String::new()),
            ...
            match result {
                WorkflowExecutionResult::Completed(_) | WorkflowExecutionResult::CompletedWithRaw { .. } => {
                    state_updater.update_workflow_complete(&final_output).await;
                    if let Some(ref cb) = workflow_callback {
                        cb.on_workflow_complete(&task_id, &final_output, total_duration, total_steps).await;
                    }
                }
                ...
                WorkflowExecutionResult::Failed { error, .. } => {
                    ...
                    if let Some(ref cb) = workflow_callback {
                        cb.on_workflow_failed(&task_id, &error, total_duration, total_steps).await;
                    }
                }
            }
        })

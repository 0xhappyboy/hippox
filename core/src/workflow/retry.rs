...
use super::types::*;
use crate::prompts::{
    build_consecutive_failures_prompt, build_error_feedback_prompt, build_max_retries_exceeded_prompt, build_timeout_feedback_prompt,
};
...
impl RetryContext {
    pub fn new(max_retries: usize, max_consecutive_failures: usize) -> Self {
        Self { max_retries, retry_counts: HashMap::new(), failed_drivers: HashSet::new(), consecutive_failures: 0, max_consecutive_failures }
    }
...
pub async fn execute_driver_with_timeout(
    ...
    .await
    {
        ...
        Err(_) => DriverExecutionResult::Timeout(format!("Driver '{}' timed out after {} seconds", call.action, timeout_secs)),
    }
}
/// Build timeout feedback for LLM
pub fn build_timeout_feedback(driver_name: &str, timeout_secs: u64, retry_count: usize, max_retries: usize) -> ChatMessage {
...
pub fn build_error_feedback(
    ...
    let prompt = build_error_feedback_prompt(driver_name, error_msg, retry_count, max_retries, &serde_json::to_value(parameters).unwrap_or_default());
    ChatMessage::user(&prompt)
}
/// Build max retries exceeded feedback for LLM
pub fn build_max_retries_exceeded_feedback(driver_name: &str, max_retries: usize, error_msg: &str) -> ChatMessage {
    let prompt = build_max_retries_exceeded_prompt(driver_name, max_retries, error_msg);
    ChatMessage::user(&prompt)
}
/// Build consecutive failures warning for LLM
pub fn build_consecutive_failures_feedback(consecutive_failures: usize, max_consecutive_failures: usize) -> ChatMessage {
...
pub fn create_step_result(call: &DriverCall, result: &DriverExecutionResult) -> StepResult {
    ...
    StepResult { driver: call.action.clone(), parameters: call.parameters.clone(), output, status }
}
/// Create a failed step result from an error
pub fn create_failed_step_result(call: &DriverCall, error: &str) -> StepResult {
    StepResult { driver: call.action.clone(), parameters: call.parameters.clone(), output: error.to_string(), status: ExecutionStatus::Failure }
}
/// Create a success step result
pub fn create_success_step_result(call: &DriverCall, output: &str) -> StepResult {
    StepResult { driver: call.action.clone(), parameters: call.parameters.clone(), output: output.to_string(), status: ExecutionStatus::Success }
}

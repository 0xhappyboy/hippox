...
pub fn workflow_mode_description_zh(mode: WorkflowMode) -> &'static str {
    match mode {
        WorkflowMode::ReAct => "思考 → 行动 → 观察循环模式，每次执行后由 LLM 决策下一步，最适合开放式任务、动态决策和错误恢复",
        WorkflowMode::Batch => "批量并行执行多个独立的驱动，驱动之间无依赖关系，最适合批量处理和独立操作",
        WorkflowMode::Chain => "链式串行执行，驱动间可传递变量，最适合线性流水线和数据转换链",
        WorkflowMode::PlanAndExecute => "先规划后执行模式，支持条件判断、变量引用和错误处理，最适合复杂工作流和确定性任务",
    }
}

...
pub enum TaskEventType {
    ...
    StepChanged { step_index: usize, step_name: String, status: StepStatus, output: Option<String>, error: Option<String>, duration_ms: Option<u64> },
    /// Task progress update
    Progress { progress: u8, message: String },
}
impl TaskLifecycleEvent {
    pub fn created(task_id: String) -> Self {
        Self { task_id, event_type: TaskEventType::Created, data: serde_json::Value::Null }
    }
    pub fn status_changed(task_id: String, status: TaskStatus) -> Self {
        Self { task_id, event_type: TaskEventType::StatusChanged { status }, data: serde_json::Value::Null }
    }
    pub fn step_changed(
        task_id: String,
        step_index: usize,
        ...
        Self {
            task_id,
            event_type: TaskEventType::StepChanged { step_index, step_name, status, output, error, duration_ms },
            data: serde_json::Value::Null,
        }
    }
    pub fn progress(task_id: String, progress: u8, message: String) -> Self {
        Self { task_id, event_type: TaskEventType::Progress { progress, message }, data: serde_json::Value::Null }
    }
...
pub static TASK_POOL_EVENT_BUS: Lazy<Arc<broadcast::Sender<TaskLifecycleEvent>>> = Lazy::new(|| {
...
impl TaskEventSubscriber {
    ...
    pub fn new() -> Self {
        Self { receiver: TASK_POOL_EVENT_BUS.subscribe() }
    }
    ...
    pub async fn wait_for_status(&mut self, target_task_id: &str, target_status: TaskStatus) -> Option<TaskLifecycleEvent> {
        while let Some(event) = self.recv().await {
            if event.task_id != target_task_id {
                continue;
            }

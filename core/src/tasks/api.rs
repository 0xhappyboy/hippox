...
use crate::{HippoxBatchResult, HippoxBoolResult, HippoxResult, HippoxStringResult, HippoxVoidResult};
use std::sync::Arc;
...
pub async fn create_task_with_executable(task_type: String, input: String, executable: Arc<dyn ExecutableTask>) -> HippoxStringResult {
    let mut pool = TASK_POOL.write().await;
...
pub async fn update_task_status(task_id: &str, status: TaskStatus) -> HippoxBoolResult {
    ...
    if success { HippoxResult::ok(true) } else { HippoxResult::system_error(format!("Failed to update task status: {}", task_id)) }
}
/// Cancel a task
pub async fn cancel_task(task_id: &str) -> HippoxBoolResult {
    let mut pool = TASK_POOL.write().await;
    let success = pool.cancel_task(task_id);
    if success { HippoxResult::ok(true) } else { HippoxResult::system_error(format!("Failed to cancel task: {}", task_id)) }
}
/// Pause a task
pub async fn pause_task(task_id: &str) -> HippoxBoolResult {
    let mut pool = TASK_POOL.write().await;
    let success = pool.pause_task(task_id);
    if success { HippoxResult::ok(true) } else { HippoxResult::system_error(format!("Failed to pause task: {}", task_id)) }
}
...
pub async fn retry_task(task_id: &str) -> HippoxBoolResult {
    ...
    if success { HippoxResult::ok(true) } else { HippoxResult::system_error(format!("Failed to retry task: {}", task_id)) }
}
/// Get all tasks
        ...
        match status {
            TaskStatus::Completed => {
                ...
                match get_task(task_id).await {
                    HippoxResult { data: Some(task), .. } => {
                        let output = task.final_output.unwrap_or_default();
                        return HippoxResult::ok_with_tokens(output, task.input_token_count, task.output_token_count);
                    }
                    HippoxResult { error: Some(e), .. } => {
                        return HippoxResult::system_error(format!("Task completed but data retrieval failed: {}", e));
                    }
                    _ => {
                        return HippoxResult::system_error(format!("Task completed but data not found: {}", task_id));
                    }
                }
            }
